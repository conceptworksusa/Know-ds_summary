...
# Welcome message
header.show_sidebar_logo("This is a Summarization Assistant!!")
...
with st.sidebar:
    contain = st.container(height=200, border=True)
    ...
    st.session_state.op_key = contain.button("Summarize")
...
if st.session_state.op_key:
    ...
    try:
        ...
        with st.chat_message("user"):
            st.markdown(f"****System Prompt:*** {system_prompt}")

        # Add user message to chat history
        st.session_state.msg_key.append({"role": "user", "content": system_prompt})

        if st.session_state.doc_id:
            response = TextSummarizer().summarize(doc_id=st.session_state.doc_id, llama_model=st.session_state.model)
